...
parser.add_argument("--notype", type=str, default='random', help='noise type; random; ')
parser.add_argument("--no_to_snr", type=float, default='3', help='add noise to degrade data quality to specified SNR value')

parser.add_argument("--folder", type=str, default='./sdata/test_3d', help='test data folder')
...
parser.add_argument("--tl", type=bool, default=False, help="是否迁移学习")
parser.add_argument("--tlfolder", type=str, default='', help="迁移学习模型所在文件夹，只有在--tl为True时有效")
parser.add_argument("--ntype", type=str, default='3d', help="网络类型")
parser.add_argument("--norm", type=str, default="absmax", help="")
...
def main():
    ...
    with torch.no_grad():
        ...
        for f in files_source:
            ...
            if opt.notype == 'random':  # 添加随机噪声
                print(f"Adding noise to data to decrease SNR value to: {opt.no_to_snr}")
                noisy_arr, noise = add_gaussian_noise(arr, snr_db=opt.no_to_snr)

            elif opt.notype == 'none':
                noisy_arr = arr.copy()
                mask = noisy_arr != 0
            ...
            if opt.testtype == 0:  # 直接输入整幅，适用于CNN-based
                if "UNet" in opt.net:
                    ...
                    noisy_arr = pad_3d_reflect(noisy_arr, 32, 32, 32)
                    print(f"After padding: {arr.shape}")
                with torch.no_grad():  # this can save much memory
                    noisy_arr = torch.tensor(noisy_arr, dtype=torch.float32).unsqueeze(0).unsqueeze(0).to(device)
                    denoised = model(noisy_arr).cpu().detach().numpy().squeeze()
                noisy_arr = noisy_arr.cpu().detach().numpy().squeeze()

                match opt.norm:
                    case 'absmax':
                        arr = arr * arr_max
                        noisy_arr = noisy_arr * arr_max
                        denoised = denoised * arr_max
                    case 'log1p':
                        arr = denorm_by_log(arr)
                        noisy_arr = denorm_by_log(noisy_arr)
                        denoised = denorm_by_log(denoised)
                    case 'TPNN':
                        arr = denorm_by_arcsinh_std(arr, arr_mean, arr_std)
                        noisy_arr = denorm_by_arcsinh_std(noisy_arr, arr_mean, arr_std)
                        denoised = denorm_by_arcsinh_std(denoised, arr_mean, arr_std)
                    case _:
                        pass

                arr = arr[:n_times, :n_inline, :n_xline]
                noisy_arr = noisy_arr[:n_times, :n_inline, :n_xline]
                denoised = denoised[:n_times, :n_inline, :n_xline]
                ...
                noisy_arr = pad_3d_reflect(noisy_arr, *block_shape)
                ...
                if opt.net not in CNN_based:
                    with torch.no_grad():  # this can save much memory
                        print("Predicting...")
                        denoised = test_with_sliding_window_3d(
                            noisy_arr, block_shape, model, device, tuple(x // 2 for x in block_shape))
                    print(f"denoised.shape: {denoised.shape}")
                else:
                    miss_arr_t = torch.tensor(noisy_arr, dtype=torch.float32).unsqueeze(0).unsqueeze(0).to(device)
                    denoised = model(miss_arr_t).cpu().detach().numpy().squeeze()
                    # denoised = np.zeros(arr.shape)
                    ...
                    #     denoised[:, i:i + block_shape[1], :] = recon
                ...
                noisy_arr = np.copy(arr)

                if opt.notype == 'regular':  # 常规缺失：每2道删除1道；每2条线删除1条
                    for xline in range(0, noisy_arr.shape[2], 3):
                        noisy_arr[:, :, xline] = 0
                    for line in range(0, noisy_arr.shape[0], 3):
                        noisy_arr[line, :, :] = 0
                elif opt.notype == 'irregular':  # 非常规缺失：随机删除道数。为了复现，人工设置随机种子
                    random.seed(98438)  # 98438： 256道合成数据、kerry数据的均匀种子；903： 拖缆数据的均匀种子；
                    del_num1 = int(noisy_arr.shape[2] * 0.5)  # 删除道
                    del_num2 = int(noisy_arr.shape[0] * 0.5)  # 删除测线
                    del_xline = random.sample(range(noisy_arr.shape[2]), del_num1)
                    del_inline = random.sample(range(noisy_arr.shape[0]), del_num2)
                    # print(sorted(del_cols))
                    for xline in del_xline:
                        noisy_arr[:, :, xline] = 0
                    for line in del_inline:
                        noisy_arr[line, :, :] = 0
                miss_t = torch.tensor(noisy_arr, dtype=torch.float32).unsqueeze(0).unsqueeze(0).to(device)
                with torch.no_grad():  # this can save much memory
                    denoised = model(miss_t).cpu().detach().numpy().squeeze()

                arr = (arr * arr_max)[:n_times, :n_inline, :n_xline]
                noisy_arr = (noisy_arr * arr_max)[:n_times, :n_inline, :n_xline]
                denoised = (denoised * arr_max)[:n_times, :n_inline, :n_xline]
                ...
                noisy_arr = pad_3d(noisy_arr, block_shape[0], block_shape[1], block_shape[2])
                print("After padding: ", arr.shape)
                ...
                if opt.net not in CNN_based:
                    with torch.no_grad():  # this can save much memory
                        ...
                        denoised = test_with_sliding_window_2d_from_3d(
                            noisy_arr, block_shape, model, device, tuple(x // 2 for x in block_shape))
                    print(f"denoised.shape: {denoised.shape}")
                else:
                    miss_arr_t = torch.tensor(noisy_arr, dtype=torch.float32).unsqueeze(0).unsqueeze(0).to(device)
            ...
            # 保存为mat文件
            result_dir = f'./results/{model_folder}/{opt.net}/{os.path.basename(opt.folder)}_denoising/'
            os.makedirs(result_dir, exist_ok=True)
            sio.savemat(os.path.join(result_dir, f'{os.path.basename(f)[:-4]}.mat'),
                        {'complete': arr, 'missing': noisy_arr,
                         'reconstructed': denoised, 'difference': denoised - arr})

            # 衡量数值性能
            snr_miss = array_SNR(noisy_arr, arr)
            psnr_miss = array_PSNR(noisy_arr, arr)
            rmse_miss = array_RMSE(noisy_arr, arr)


            snr_test = array_SNR(denoised, arr)
            psnr_test = array_PSNR(denoised, arr)
            rmse_test = array_RMSE(denoised, arr)

            shape = np.array(arr.shape)
            candidates = np.where(shape < 7)[0]
            if len(candidates) == 0:
                ssim_miss, _ = ssim(noisy_arr, arr, full=True, data_range=arr_max)
                ssim_test, _ = ssim(denoised, arr, full=True, data_range=arr_max)
            else:
                dim = candidates[np.argmin(shape[candidates])]
                size = arr.shape[dim]
                odd = size - 1 if size % 2 == 0 else size - 2
                odd = max(odd, 1)
                ssim_miss, _ = ssim(noisy_arr, arr, full=True, data_range=arr_max, win_size=odd)
                ssim_test, _ = ssim(denoised, arr, full=True, data_range=arr_max, win_size=odd)

            end_time = time.time()
            print(f"Inference time of {fname}: {end_time - start_time}s")

            with open(os.path.join(result_dir, f'metrics_{opt.folder}_denoising.log'), 'w') as result_file:
                ...
                print(f"SNR on denoised {os.path.basename(f)}: {snr_test: .2f}")
                print(f"PSNR on denoised {os.path.basename(f)}: {psnr_test: .2f}")
                print(f"RMSE on denoised {os.path.basename(f)}:  {rmse_test: .4f}")
                print(f"SSIM on denoised {os.path.basename(f)}: {ssim_test: .6f}")
                ...
                result_file.write(f"SNR on denoised {os.path.basename(f)}:  {snr_test: .2f}\n")
                result_file.write(f"PSNR on denoised {os.path.basename(f)}:  {psnr_test: .2f}\n")
                result_file.write(f"RMSE on denoised {os.path.basename(f)}:  {rmse_test: .4f}\n")
                result_file.write(f"SSIM on denoised {os.path.basename(f)}: {ssim_test: .6f}\n")

                print("----------------------------------------------------------")
                for i in range(0, arr.shape[2]):
                    arr_i = arr[:, :, i]
                    miss_arr_i = noisy_arr[:, :, i]
                    reconstructed_i = denoised[:, :, i]
                    ...
                    print(f"SNR on denoised {os.path.basename(f)}, slice {i}:  {snr_test: .2f}")
                    print(f"PSNR on denoised {os.path.basename(f)}, slice {i}:  {psnr_test: .2f}")
                    print(f"RMSE on denoised {os.path.basename(f)}, slice {i}:  {rmse_test: .4f}")
                    print(f"SSIM on denoised {os.path.basename(f)}, slice {i}: {ssim_test: .6f}")
                    print()
                    ...
                    result_file.write(f"SNR on denoised {os.path.basename(f)}, slice {i}:  {snr_test: .2f}\n")
                    result_file.write(f"PSNR on denoised {os.path.basename(f)}, slice {i}:  {psnr_test: .2f}\n")
                    result_file.write(f"RMSE on denoised {os.path.basename(f)}, slice {i}:  {rmse_test: .4f}\n")
                    result_file.write(f"SSIM on denoised {os.path.basename(f)}, slice {i}: {ssim_test: .6f}\n")
                print("----------------------------------------------------------")
